...
const FEATURE_MAP: &[FeatureInfo] = &[
    ...
    FeatureInfo {
        ...
        status: "Shared JSON health helpers, std HTTP, graceful health-pool startup, route/request/lifecycle/duration/middleware marker headers, packaged Axum/Actix adapters, checked services, installed scaffolds, and route smoke coverage exist; live route checks remain planned",
    },
    FeatureInfo {
        stream: "10",
...
const GEMSTONE_RS_PARITY: &[ParityInfo] = &[
    ...
    ParityInfo {
        ...
        project_score: 4,
        leader: "gemstone-py",
        status: "gemstone-py has broader FastAPI, Litestar, and Django examples; gemstone-rs has shared health helpers, Axum/Actix adapters, request-trace headers, lifecycle-duration headers, middleware markers, and stricter route smoke tests.",
        next_action: "Add live route smoke tests and broader production middleware patterns for Axum and Actix.",
    },
    ParityInfo {
    ...
    ParityInfo {
        ...
        next_action: "Polish live class browsing, generated-file editing, screenshots, and browser fallback behavior inside the embedded webview.",
    },
    ParityInfo {
...
const GEMSTONE_PY_GAPS: &[GapInfo] = &[
    GapInfo {
        ...
        next_action: "Add live route smoke tests and broader production middleware patterns for the packaged Axum/Actix adapters.",
    ...
    GapInfo {
        ...
        gemstone_rs_gap: "gemstone-rs-explorer and the VS Code webview now cover structured setup checks, profile status, codegen summaries/diffs, generated output reads, BridgeRoot keys/values, generated-file actions, and comparison status. They still need more polished live class browsing, screenshots, generated-file editing, and browser fallback behavior.",
        next_action: "Polish live class browsing, generated-file editing, screenshots, and browser fallback behavior inside the embedded webview.",
...
const GEMSTONE_RS_BATCHES: &[BatchInfo] = &[
    BatchInfo {
        ...
        hours_min: 5,
        hours_max: 8,
        outcome: "Polish live class browsing, generated-file editing, screenshots, and browser fallback behavior inside the embedded webview.",
        verify_with: "python3 scripts/explorer_endpoint_smoke.py; vscode-gemstone-rs-workbench smoke test",
    },
    BatchInfo {
        number: 2,
    ...
    BatchInfo {
        ...
        hours_min: 4,
        hours_max: 7,
        outcome: "Layer a cautious async facade over SessionWorkerPool and add live Axum/Actix route smoke coverage plus broader production middleware patterns.",
...
mod tests {
    ...
    fn comparison_batch_plans_are_actionable() {
        ...
        assert_eq!(total_batch_hours(GEMSTONE_RS_BATCHES), (37, 64));
        ...
        assert!(scorecard_json_entry(gemstone_py_scorecard_info())
            .contains(r#""remaining":{"totalBatches":6,"hoursMin":37,"hoursMax":64}"#));
        assert!(
            !status_json_entry(gemstone_py_scorecard_info(), GEMSTONE_RS_PARITY)
                .contains(r#""view":"#)
        );
    ...
    fn gap_report_prioritizes_actionable_gemstone_py_gaps() {
        ...
        assert!(GEMSTONE_PY_GAPS
            ...
            .any(|gap| gap.area == "Explorer product polish"
                ...
                && gap.next_action.contains("browser fallback behavior")));
