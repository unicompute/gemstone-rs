...
fn print_routes(options: &Options) {
    ...
    println!("Start:");
    println!(
        "  cargo run --manifest-path examples/actix-service/Cargo.toml -- --host {} --port {}",
...
fn print_usage() {
    println!(
        "usage: cargo run --manifest-path examples/actix-service/Cargo.toml -- [--host <host>] [--port <port>] [--routes]"
    );
}
