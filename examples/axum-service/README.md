...
The service starts through `gemstone_rs_axum::health_pool_from_env`, then passes
that health backend to `gemstone_rs_axum::router_with_health_pool`. That lets
the server boot before credentials are configured: `/` and `/health/local`
