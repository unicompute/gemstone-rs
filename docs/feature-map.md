...
| 5 | BridgeRoot object mapping | `gemstone-rs::bridge`, `gemstone-rs-macros` | `bridge_root_mapping`, `derive_mapping`, `bridge_value_inspection`, `remote_object_mapping`, `generated_mapping_app` | `docs/object-mapping.md`, `docs/cookbook.md` | `SmalltalkBridge`, `PersistentRoot`, facade examples | Rust has typed mapping, derive, nested dynamic `BridgeValue` read-back, `Remote<T>` / `ObjectRef<T>` handles, materialization profiles, repeated-OOP identity groups, and path-aware diagnostics. It intentionally stays explicit instead of becoming transparent persistence. |
