...
reports **11 batches** and roughly **72-125 hours** across the Rust and
...
| 7 | Explorer workflow | `gemstone-rs-explorer` | `tooling/explorer.md` | `docs/explorer.md`, `docs/screenshots.md` | `python-gemstone-database-explorer` | Useful local UI/API with setup, profile status, live browse path/source previews, editable generated output, BridgeRoot key/value summaries, codegen workflows, VS Code open-file actions, browser fallback prompts, and committed visual assets; Python explorer remains the richer product reference. |
| 8 | VS Code workbench | `vscode-gemstone-rs-workbench` | `tooling/vscode-workbench.md` | `docs/vscode-workbench.md` | `gemstone-py Workbench` | Command workflow and embedded webview now render setup checks, profile status, codegen summaries/diffs/editable output files, BridgeRoot keys/values, comparison status, and Marketplace/GitHub visuals. |
