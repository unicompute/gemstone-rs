...
standard HTTP, Axum, and Actix workflows. JSON output is intended for CI, docs
...
| 2 | Safe sessions and transactions | `gemstone-rs::Session`, `SessionWorker`, `TransactionGuard` | `quickstart`, `transactions`, `session_worker`, `live_smoke_cookbook` | `docs/user-manual.md`, `docs/cookbook.md` | `GemStoneSession`, `SessionFacade`, transaction policies | Core parity for sync eval, perform, commit, and abort, plus a dedicated-thread worker for web/async callers. |
...
| 9 | Rust web services | `gemstone-rs::SessionWorker` plus checked Axum/Actix services and installed scaffolds | `session_worker`, `http_service`, `examples/axum-service`, `examples/actix-service`, `examples scaffold axum_service/actix_service` | `docs/examples-guide.md`, `docs/cookbook.md` | FastAPI, Litestar, Django examples | Standard-library HTTP, SessionWorker, checked Axum/Actix services, and installed scaffolds exist; reusable framework adapters remain planned. |
