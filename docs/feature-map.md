...
reports **11 batches** and roughly **67-117 hours** across the Rust and
...
| 5 | BridgeRoot object mapping | `gemstone-rs::bridge`, `gemstone-rs-macros` | `bridge_root_mapping`, `derive_mapping`, `bridge_value_inspection`, `generated_mapping_app` | `docs/object-mapping.md`, `docs/cookbook.md` | `SmalltalkBridge`, `PersistentRoot`, facade examples | Rust has typed mapping, derive, nested dynamic `BridgeValue` read-back, and path-aware diagnostics; a transparent object model remains future work. |
