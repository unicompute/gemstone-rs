...
| 9 | Rust web services | `gemstone-rs::web`, `SessionWorkerPool`, checked Axum/Actix services, and installed scaffolds | `session_worker`, `session_worker_pool`, `http_service`, `examples/axum-service`, `examples/actix-service`, `examples scaffold session_worker_pool/axum_service/actix_service` | `docs/examples-guide.md`, `docs/cookbook.md` | FastAPI, Litestar, Django examples | Shared JSON health helpers, standard-library HTTP, SessionWorkerPool, checked Axum/Actix services, and installed scaffolds exist; packaged framework adapters remain planned. |
