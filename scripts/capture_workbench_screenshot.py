#!/usr/bin/env python3
"""Capture deterministic gemstone-rs Workbench visuals for Marketplace docs."""
...
GIF_OUTPUT = ROOT / "docs" / "assets" / "workbench-codegen-edit-flow.gif"
...
button.active { background: var(--accent); box-shadow: 0 0 0 2px rgba(208,68,55,.28); }
...
<div class="shell">
  ...
  <div class="layout">
    <aside class="rail">
      ...
      <div class="group">
        <h2>Codegen</h2>
        <button class="active">Preview/Edit Generated Wrappers</button>
...
def main() -> int:
    with tempfile.TemporaryDirectory(prefix="gemstone-rs-workbench-shot-") as tmp:
        ...
        for index, frame in enumerate(frames()):
            html = tmp_path / f"workbench-codegen-edit-flow-{index}.html"
            image = tmp_path / f"workbench-codegen-edit-flow-{index}.png"
            html.write_text(frame["html"], encoding="utf-8")
            capture(html.as_uri(), OUTPUT if index == 0 else image, 1440, 900)
            frame_paths.append(OUTPUT if index == 0 else image)
        write_gif(frame_paths, GIF_OUTPUT)
    print(OUTPUT)
    print(GIF_OUTPUT)
    return 0


def frames() -> list[dict[str, str]]:
    return [
        {
            "html": render_html(
                open_output_button='<button class="secondary">Open Output File</button>',
                draft_button='<button class="secondary">Open Editable Draft</button>',
                save_button="<button>Save Edited Output</button>",
                state_text="Edited in webview. Save writes the configured generated output file after confirmation.",
            )
        },
        {
            "html": render_html(
                open_output_button='<button class="secondary">Open Output File</button>',
                draft_button='<button class="active">Open Editable Draft</button>',
                save_button='<button class="secondary">Save Edited Output</button>',
                state_text="<strong>Draft opened.</strong> Review generated Rust in a normal VS Code editor before writing.",
            )
        },
        {
            "html": render_html(
                open_output_button='<button class="secondary">Open Output File</button>',
                draft_button='<button class="secondary">Open Editable Draft</button>',
                save_button='<button class="active">Save Edited Output</button>',
                state_text="<strong>Saved 1,284 bytes.</strong> The edited wrappers were written after confirmation.",
            )
        },
    ]
...
def render_html(
    *,
    ...
    return (
        HTML.replace("{open_output_button}", open_output_button)
        .replace("{draft_button}", draft_button)
        .replace("{save_button}", save_button)
        .replace("{state_text}", state_text)
    )


def write_gif(frame_paths: list[Path], output: Path) -> None:
